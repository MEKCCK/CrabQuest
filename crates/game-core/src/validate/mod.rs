        ...
        CompileOutcome::Success { binary } => {
            ...
            match sandbox.run(&binary)? {
                ...
                RunOutcome::Panic { message } => {
                    ...
                    if !level.expect_panic.is_empty() {
                        // expect_panic 子串判定（P3-22）：净化后消息包含子串（大小写敏感）→ 通关
                        if sp.message.contains(&level.expect_panic) {
                            Ok(Validation::Pass)
                        } else {
                            Ok(Validation::Fail {
                                feedback: vec![format!(
                                    "panic 消息不匹配：期望包含「{}」，实际为：\n{}",
                                    level.expect_panic, sp.message
                                )],
                            })
                        }
                    } else {
...
mod tests {
    ...
    #[test]
    fn sanitize_panic_strips_noise() {
        // 直接验证 P1-01 净化（sanitize_panic）：行首空白、临时目录路径、thread 头与线程 id、note 行全部剥除；
        // 保留 main.rs:N:M 定位行与消息体
        let raw = "\nthread 'main' (535665) panicked at /tmp/rlg-Ab12Cd34/main.rs:3:24:\nindex out of bounds: the len is 3 but the index is 3\nnote: run with `RUST_BACKTRACE=1` environment variable to display a backtrace\n";
        let sp = sanitize_panic(raw);
        assert!(sp.message.contains("main.rs:3:24:"), "message: {sp:?}");
        assert!(sp.message.contains("index out of bounds: the len is 3 but the index is 3"), "message: {sp:?}");
        assert!(!sp.message.contains("/tmp/"), "message: {sp:?}");
        assert!(!sp.message.contains("rlg-"), "message: {sp:?}");
        assert!(!sp.message.contains("thread 'main'"), "message: {sp:?}");
        assert!(!sp.message.contains("535665"), "message: {sp:?}");
        assert!(!sp.message.contains("note:"), "message: {sp:?}");
        assert_eq!(sp.line, Some(3), "应解析出定位行号: {sp:?}");
    }
}
